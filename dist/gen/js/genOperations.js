...
function renderDestructuredParamSignature(op, options, pkg, bare = false) {
    ...
    if (!destructuredParams)
        return;
    if (options.language === 'ts')
        return `{ ${destructuredParams} }: { ${signature} }`;
    return `{ ${destructuredParams} }`;
}
exports.renderDestructuredParamSignature = renderDestructuredParamSignature;
function renderRequiredParamsSignature(required, options, typed = true) {
    return required.reduce((a, param) => {
        a.push(getParamSignature(param, options, typed));
        return a;
    }, []);
}
function renderOptionalParamsSignature(op, optional, options, pkg, typed = true) {
    if (!optional.length)
        return [];
    if (!pkg)
        pkg = '';
...
function getParamSignature(param, options, typed = true) {
    const signature = [getParamName(param.name)];
    if (options.language === 'ts' && typed)
